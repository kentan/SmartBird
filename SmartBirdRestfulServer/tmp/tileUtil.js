...
//$(document).ready(function(){
	...
	var operationFunc = {"add":_refreshTiles,
			...
			"initTiles":_initTiles,
			"roundFinished":_finishRound,
	};
	...
	function _refreshTiles(tiles){
		$('#log').append("add:"+ tiles.wall +"<br/>");
//				addTile(tile,13);
		clearAllTiles();
		_initTiles(tiles.wall);
	}
	function drawTile(tile,pos,emphasize){
		if(pos > 14){
			return ;
		}
 		tileFunc[tile]("myCanvas"+pos);
    }
	function drawDiscardedTile(tile){
		if(dt_pos > 24){
			return ;
		}
		tileFunc[tile]("dt"+dt_pos);
		dt_pos++;
	}	
	
	function _discardTile(tile){

		var posOfTheTile = tilePos[tile];
		clearTile("myCanvas"+posOfTheTile);
		drawDiscardedTile(tile);
		$('#log').append("discard:"+ tile +":" + posOfTheTile + "<br/>");
	}
	function drawAllTiles(tiles){
		$('#log').append(tiles + "<br/>");
//				if(done) return;
		for(var i=0; i < tiles.length; i++){
			var tile = tiles[i];
			$('#log').append(tiles[i] + "<br/>");
			drawTile(tile,i);
			tilePos[tile] = i;
		}
		$('#log').append(tilePos);	
	...
	function _finishRound(value){
		clearAllTiles();
		clearAllDiscardedTiles();
	}
  	function getNextMessage(){
    	var host="http://localhost:8080/SmartBirdRestfulServer/endpoint/next";;
    	$.ajax({
    	    type: 'GET',
    	    url: host,
    	    success: function(d) {
  	...
  	function init(){
    	var host="http://localhost:8080/SmartBirdRestfulServer/endpoint/start";;
